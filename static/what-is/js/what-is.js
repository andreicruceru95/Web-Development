//hide and display div content
function what(){
    div = document.getElementById('ml2');
    div.style.display = "block";
    div2 = document.getElementById('ml1');
    div2.style.display = "none";
    div3 = document.getElementById('ml3');
    div3.style.display = "none";
    div4 = document.getElementById('ml4');
    div4.style.display = "none";
    div5 = document.getElementById('ml5');
    div5.style.display = "none";
    div6 = document.getElementById('ml6');
    div6.style.display = "none";
}
function why(){
    div = document.getElementById('ml1');
    div.style.display = "block";
    div2 = document.getElementById('ml2');
    div2.style.display = "none";
    div3 = document.getElementById('ml3');
    div3.style.display = "none";
    div4 = document.getElementById('ml4');
    div4.style.display = "none";
    div5 = document.getElementById('ml5');
    div5.style.display = "none";
    div6 = document.getElementById('ml6');
    div6.style.display = "none";    
}
function foundations(){
    div = document.getElementById('ml3');
    div.style.display = "block";
    div2 = document.getElementById('ml1');
    div2.style.display = "none";
    div3 = document.getElementById('ml2');
    div3.style.display = "none";
    div4 = document.getElementById('ml4');
    div4.style.display = "none";
    div5 = document.getElementById('ml5');
    div5.style.display = "none";
    div6 = document.getElementById('ml6');
    div6.style.display = "none";   
}
function relations(){
    div = document.getElementById('ml4');
    div.style.display = "block";
    div2 = document.getElementById('ml1');
    div2.style.display = "none";
    div3 = document.getElementById('ml2');
    div3.style.display = "none";
    div4 = document.getElementById('ml3');
    div4.style.display = "none";
    div5 = document.getElementById('ml5');
    div5.style.display = "none";
    div6 = document.getElementById('ml6');
    div6.style.display = "none";   
}
function early(){
    div = document.getElementById('ml5');
    div.style.display = "block";
    div2 = document.getElementById('ml1');
    div2.style.display = "none";
    div3 = document.getElementById('ml2');
    div3.style.display = "none";
    div4 = document.getElementById('ml4');
    div4.style.display = "none";
    div5 = document.getElementById('ml3');
    div5.style.display = "none";
    div6 = document.getElementById('ml6');
    div6.style.display = "none";   
}
function modern(){
    div = document.getElementById('ml6');
    div.style.display = "block";
    div2 = document.getElementById('ml1');
    div2.style.display = "none";
    div3 = document.getElementById('ml2');
    div3.style.display = "none";
    div4 = document.getElementById('ml4');
    div4.style.display = "none";
    div5 = document.getElementById('ml5');
    div5.style.display = "none";
    div6 = document.getElementById('ml3');
    div6.style.display = "none";   
}
